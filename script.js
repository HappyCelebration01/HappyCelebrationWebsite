...
// Custom JS Carousel Slider with Ambient Drift & Fast Arrow Shift Navigation
function initCarouselSlider() {
  ...
  function getStep() {
    ...
    if (!slide) return 0;
    return slide.getBoundingClientRect().width + 16; // width + gap
  }
  ...
  function updateDrift() {
    ...
    if (!isPaused) {
      // Slow drift speed: 1 slide (e.g. 508px) in 60 seconds at 60fps = ~0.14 pixels per frame
      const step = getStep();
      const driftSpeed = step / (60 * 60); // slide width / (60s * 60fps)
      currentX -= driftSpeed;
  ...
  function shiftSlide(direction) {
    ...
    setTimeout(() => {
      ...
      // Infinite loop wrap-around checks
      if (currentX <= -halfWidth) {
        currentX += halfWidth;
        track.style.transform = `translateX(${currentX}px)`;
      } else if (currentX > 0) {
